...
namespace HoYa.Controllers
{
    ...
    public class ProfilesController : ApiController
    {
        ...
        {
            IQueryable<Profile> profiles = db.Profiles.Where(x => (x.DefinitionBranch.Value.Contains(anyLike) || anyLike == null) ||
                           (x.DefinitionBranch.Code.Contains(anyLike) || anyLike == null) ||
        ...
        {
            Profile existedProfile = await db.Profiles.FindAsync(id);
            db.Entry(existedProfile).CurrentValues.SetValues(profile);
            Person existedPerson = await db.People.FindAsync(existedProfile.DefinitionBranchId);
            if(existedPerson!=null) db.Entry(existedPerson).CurrentValues.SetValues(profile.DefinitionBranch);
            await db.SaveChangesAsync();
            return Ok(existedProfile);
        }

        [Route("api/Profiles/Groups/{id}")]
